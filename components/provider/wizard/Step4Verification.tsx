...
export function Step4Verification({
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      // 2. POST binary file to storage URL
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-6 rounded-xl border bg-card p-5 shadow-xs">
        ...
        {/* CNIC Upload Dropzones */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          {/* Front Side Upload */}
          <div className="space-y-2">
            <Label className="text-xs font-semibold flex items-center justify-between">
              <span>CNIC Front Side Image <span className="text-destructive">*</span></span>
              {formData.cnicFrontStorageId && (
            ...
            <div
              className={cn(
                ...
                uploadingFront && "opacity-60 pointer-events-none"
              )}
            >
              {uploadingFront ? (
                ...
                <div className="flex flex-col items-center gap-2 py-2">
                  ...
                  <span className="text-xs font-medium text-foreground truncate max-w-[200px]">
                    {formData.cnicFrontFileName || "CNIC Front Uploaded"}
                  </span>
                  <label htmlFor="cnicFrontInput" className="text-[11px] text-primary underline cursor-pointer hover:text-primary/80">
                    Change image
                  </label>
                </div>
              ) : (
                <label htmlFor="cnicFrontInput" className="flex flex-col items-center gap-2 py-4 cursor-pointer w-full">
                  <ImageIcon className="size-8 text-muted-foreground/60" />
                  <span className="text-xs font-medium text-foreground">Upload CNIC Front</span>
            ...
            {errors.cnicFrontStorageId && (
              <p className="text-xs font-medium text-destructive">{errors.cnicFrontStorageId}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            <Label className="text-xs font-semibold flex items-center justify-between">
              <span>CNIC Back Side Image <span className="text-destructive">*</span></span>
              {formData.cnicBackStorageId && (
                <Badge variant="outline" className="text-[10px] text-emerald-600 border-emerald-500/30 bg-emerald-50 dark:bg-emerald-950/30">
                  Uploaded
                </Badge>
              )}
            ...
            <div
              className={cn(
                ...
                formData.cnicBackStorageId
                  ? "border-emerald-500/50 bg-emerald-500/5"
                  : "border-border bg-muted/20 hover:bg-muted/40",
                uploadingBack && "opacity-60 pointer-events-none"
              )}
            >
              {uploadingBack ? (
                <div className="flex flex-col items-center gap-2 py-4">
                ...
                <div className="flex flex-col items-center gap-2 py-2">
                  <FileCheck className="size-8 text-emerald-600" />
                  <span className="text-xs font-medium text-foreground truncate max-w-[200px]">
                    {formData.cnicBackFileName || "CNIC Back Uploaded"}
                  </span>
                  <label htmlFor="cnicBackInput" className="text-[11px] text-primary underline cursor-pointer hover:text-primary/80">
                    Change image
                  </label>
                </div>
              ) : (
                <label htmlFor="cnicBackInput" className="flex flex-col items-center gap-2 py-4 cursor-pointer w-full">
                  <ImageIcon className="size-8 text-muted-foreground/60" />
                  <span className="text-xs font-medium text-foreground">Upload CNIC Back</span>
                  <span className="text-[10px] text-muted-foreground">JPG, PNG, or WEBP (Max 10MB)</span>
                  <span className="mt-1 rounded-md bg-secondary px-3 py-1 text-xs font-medium text-secondary-foreground">
                    Browse File
                  </span>
            ...
            {errors.cnicBackStorageId && (
              <p className="text-xs font-medium text-destructive">{errors.cnicBackStorageId}</p>
            )}
          </div>
        </div>
      </div>
